...
const HACKING_WORDS = ["BIRTHDAY", "SYSTEMS", "TERMINAL", "COMPUTER", "OVERRIDE", "SECURITY", "DATABASE", "ARCHIVE", "MEMORIES", "HOLOTAPE", "DRONFIELD", "SEPTEMBER", REBOOT_PASSWORD];
const MEMORY_ROUNDS_TO_WIN = 3;
// ===================================
...
// --- INITIALIZATION ---
window.onload = () => {
    // The game starts by showing the "pristine" site, then breaking after 2 seconds.
    setTimeout(() => { 
    ...
    // Initialize all remaining puzzles
...
function initializeCablePuzzle() {
    ...
    function startDrag(e) {
        ...
        function move(e) {
            ...
            socket.classList.toggle('active', isOverSocket(socket, coords));
        }
        function end(e) {
            ...
            if (isOverSocket(socket, getMousePosition(svg, e))) {
...
// --- PUZZLE 4: MEMORY DEFRAGMENTATION ---
function initializeMemoryPuzzle() {
    ...
    grid.innerHTML = '';
    const blocks = [];
    let computerSequence = [], playerSequence = [], round = 0, isPlayerTurn = false;
    for(let i=0; i<9; i++) { const block = document.createElement('div'); block.className = 'memory-block'; block.dataset.index = i; block.addEventListener('click', () => { if(isPlayerTurn) playerTurn(i); }); grid.appendChild(block); blocks.push(block); }
    startButton.onclick = startGame;
    function startGame() { startButton.style.display = 'none'; status.style.display = 'block'; round = 0; computerSequence = []; nextRound(); }
    async function nextRound() { status.textContent = 'WATCH CAREFULLY...'; isPlayerTurn = false; playerSequence = []; round++; computerSequence.push(Math.floor(Math.random() * 9)); await playSequence(); status.textContent = `ROUND ${round}. REPEAT SEQUENCE.`; isPlayerTurn = true; }
    async function playSequence() { for(const index of computerSequence) { await new Promise(r => setTimeout(r, 250)); blocks[index].classList.add('lit'); await new Promise(r => setTimeout(r, 500)); blocks[index].classList.remove('lit'); } }
    function playerTurn(index) {
        blocks[index].classList.add('lit');
        setTimeout(() => blocks[index].classList.remove('lit'), 200);
        playerSequence.push(index);
        const lastIndex = playerSequence.length - 1;
        if(playerSequence[lastIndex] !== computerSequence[lastIndex]) {
            status.textContent = 'ERROR! RETRYING SEQUENCE...'; isPlayerTurn = false;
            setTimeout(async () => { playerSequence = []; await playSequence(); status.textContent = `ROUND ${round}. REPEAT SEQUENCE.`; isPlayerTurn = true; }, 1500);
            return;
        }
        if(playerSequence.length === computerSequence.length) {
            if(round >= MEMORY_ROUNDS_TO_WIN) {
                status.textContent = 'DEFRAG COMPLETE!';
                showSuccessAnimation("[ MEMORY DEFRAGMENTED ]", () => { body.className = 'state-rebooting'; });
            } else {
                isPlayerTurn = false;
...
function initializeHackingPuzzle() {
    ...
    function handleHackAttempt(e) {
        ...
        if (clickedWord === REBOOT_PASSWORD) {
            showSuccessAnimation("[ SECURITY BYPASSED ]", () => { runRebootSequence(); });
        } else {
            attemptsLeft--;
            attemptsText.textContent = `ATTEMPTS REMAINING: ${attemptsLeft}`;
            e.target.classList.add('deleted');
            if (attemptsLeft <= 0) {
                showSuccessAnimation("[ SYSTEM LOCKOUT ]", () => { initializeHackingPuzzle(); });
            }
        }
    }
}
...
function collectClue(clueElement, clueValue) {
    ...
    if (!clueElement || clueElement.classList.contains('found') || !collectedCluesDisplay) return;
...
function isOverSocket(socket, coords) {
    ...
    return coords.x > rect.x && coords.x < rect.x + rect.width && coords.y > rect.y && coords.y < rect.y + rect.height;
}